document.addEventListener('DOMContentLoaded',()=>{
    ...
    async function fetchData(){
        const res = await fetch('https://66df33eade4426916ee3e1fd.mockapi.io/Tarea');
        data=await res.json();
        return data;
    }
    function displayCapsula(capsula){
        ...
        document.querySelectorAll('.eliminado').forEach( button=>{
            button.addEventListener('click',botonelimina);
        })
    }
...
async function addNewTask(){
    ...
    await fetch('https://66df33eade4426916ee3e1fd.mockapi.io/Tarea',{
        method: 'POST',
        headers:{
...
async function botoncompletado(event) {
    ...
    await fetch(`https://66df33eade4426916ee3e1fd.mockapi.io/Tarea/${id}`,{
        method: 'PUT',
        headers:{
            'content-Type' : 'application/json',
        },
...
async function botonelimina(event1) {
    const id= event1.target.getAttribute('data-id');
    await fetch(`https://66df33eade4426916ee3e1fd.mockapi.io/Tarea/${id}`,{
        method: 'DELETE',
        headers:{
           'Content-Type' : 'application/json',
